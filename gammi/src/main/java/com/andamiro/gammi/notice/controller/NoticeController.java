...
public class NoticeController {
	...
	public String noticeNewTop3Method() throws UnsupportedEncodingException {		
		...
		for(Notice notice : list) {
			...
			jobj.put("notice_num", notice.getNotice_num());
			
			//한글에 대해서는 인코딩해서 json 에 담도록 함
			//한글이 깨지지 않음
			jobj.put("noticetitle", URLEncoder.encode(notice.getNotice_title(), "utf-8"));
			
			//날짜는 반드시 toString() 으로 문자열로 바꿔서 json 담아야 함
			jobj.put("noticedate", notice.getNotice_date().toString());
